---
layout: default
title: Barkpedia
app_icon: /assets/app-icons/icon_Barkpedia.png
app_description: "Destek"
lang: tr
permalink: /barkpedia/support/tr/
redirect_from:
  - /support-url/barkpedia/tr/
---

# Size nasıl yardımcı olabiliriz?

Barkpedia ile harika bir deneyim yaşamanızı istiyoruz. Bir şey beklendiği gibi çalışmıyorsa, yanıtı büyük olasılıkla aşağıda bulacaksınız. Bulamazsanız, bir e-posta kadar yakınız.

## Başlarken

### Irk tanımlama nasıl çalışır?

Barkpedia, fotoğrafınızı analiz edip köpek ırkını tanımlamak için gelişmiş cihaz üzeri zekayı kullanır. Tüm işlemler iPhone veya iPad'inizde yerel olarak gerçekleşir, bu nedenle fotoğraflarınız asla hiçbir yere yüklenmez.

### İnternet bağlantısına ihtiyacım var mı?

Temel deneyim için gerekmez. Irk tanımlama, kaydedilen favoriler, ırk arama, dünya haritası ve Köpek Bilgi Yarışması tamamen çevrimdışı çalışır. Irk ayrıntı sayfaları (mizaç, tarihçe, fotoğraflar) ve Irk Yarışması içeriği yüklemek için internet bağlantısı gerektirir.

### Barkpedia gerçekten ücretsiz mi?

Evet, hiçbir gizli koşul olmadan tamamen ücretsiz. Uygulama içi satın alma, abonelik veya reklam yoktur.

### Barkpedia kaç ırkı tanıyabilir?

Barkpedia, AKC standartlarına dayanarak 147 ırkı tanımlar.

## Uygulamayı Kullanma

### Keşfet sekmesi nedir?

Keşfet, ırkları keşfetmenin iki yolunu sunar:

- **Arama** — 147 ırkın tamamında anında filtreleme için herhangi bir ırk adı yazın.
- **Dünya Haritası** — Irk kökenlerini ülkeye göre gösteren etkileşimli bir haritaya göz atın. O bölgeden hangi ırkların geldiğini görmek için bir işaretçiye dokunun. Haritayı tam ekran görüntülemek için genişletme düğmesini kullanın.

Hem arama hem de harita çevrimdışı çalışır.

### Sınav ve Bilgi Yarışması oyunları nasıl çalışır?

- **Irk Yarışması** — Size bir köpek fotoğrafı gösterilir ve dört seçenekten doğru ırkı seçersiniz. Her turda 10 soru vardır. Puanınıza göre ödül unvanları kazanın.
- **Köpek Bilgi Yarışması** — Gerçek köpek gerçeklerini akla yatkın efsanelerle karıştıran doğru-yanlış ifadeleri. Bu da tur başına 10 soru, ödül unvanlarıyla.

Irk Yarışması, fotoğrafları yüklemek için internet bağlantısı gerektirir. Köpek Bilgi Yarışması tamamen çevrimdışı çalışır.

### Barkpedia iPad'de çalışır mı?

Evet. Barkpedia düzenini otomatik olarak uyarlar; iPhone'da sekme gezinmesi, iPad'de kenar çubuğu gezinmesi kullanılır. Beş bölümün tümü (Tara, Favoriler, Keşfet, Sınav ve Bilgi Yarışması) her iki cihaz için tamamen optimize edilmiştir.

## Gizlilik ve Veriler

### Verilerim gizli mi?

Evet. Fotoğraflarınız tamamen cihazınızda işlenir ve asla yüklenmez. Barkpedia hiçbir kişisel veri toplamaz, kullanıcı hesabı yoktur ve hiçbir analiz veya izleme içermez. Ayrıntılar için tam [Gizlilik Politikamıza](https://www.bartlettbutter.com/barkpedia/privacy/) bakın.

### Verilerim nerede saklanır?

Tarama geçmişi ve favoriler yalnızca cihazınızda yerel olarak saklanır. Hiçbir şey harici sunuculara gönderilmez.

### Verilerimi nasıl silerim?

Kaldırmak için Favoriler sekmesindeki herhangi bir öğeyi sola kaydırın. Tüm uygulama verilerini bir kerede silmek için Barkpedia'yı cihazınızdan silmeniz yeterlidir.

### Uygulamayı silersem ne olur?

Barkpedia'yı kaldırmak, kaydedilen tüm favorileri ve tarama geçmişini cihazınızdan kalıcı olarak kaldırır. Bulut yedeklemesi veya senkronizasyonu olmadığından bu işlem geri alınamaz.

## İzinler

### Barkpedia neden kamera erişimine ihtiyaç duyuyor?

Kamera erişimi, ırk tanımlama için köpekleri doğrudan uygulama içinde fotoğraflamanızı sağlar. Bu izin isteğe bağlıdır. Bunun yerine fotoğraf kitaplığınızı kullanabilirsiniz.

### Barkpedia neden fotoğraf kitaplığı erişimine ihtiyaç duyuyor?

Fotoğraf kitaplığı erişimi, tanımlama için mevcut fotoğrafları seçmenizi sağlar. Bu, kamerayı kullanmanın bir alternatifidir.

### Barkpedia konumuma veya diğer verilere erişiyor mu?

Hayır. Barkpedia yalnızca kamera ve fotoğraf kitaplığı erişimi ister. Konum hizmetlerini, kişileri, mikrofonu, bildirimleri veya diğer hassas izinleri kullanmaz.

## Sorun Giderme

### Kamera çalışmıyor

Ayarlar → Barkpedia'ya gidin ve Kamera erişiminin etkin olduğunu doğrulayın. Zaten açıksa, uygulamayı zorla kapatıp yeniden açmayı deneyin.

### Irk ayrıntı veya sınav içeriği yüklenmiyor

Irk ayrıntı sayfaları ve Irk Yarışması etkin bir internet bağlantısı gerektirir. Wi-Fi veya hücresel bağlantınızı kontrol edip yeniden deneyin. Sorun devam ederse içerik hizmeti geçici olarak kullanılamıyor olabilir. Birkaç dakika bekleyip yeniden deneyin. (Köpek Bilgi Yarışması çevrimdışı çalışır ve bundan etkilenmemelidir.)

### Uygulama "Köpek Değil" diyor

Barkpedia, ırkı tanımlamadan önce fotoğrafta bir köpek olduğunu doğrular. Görüntü çok bulanıksa, kötü aydınlatılmışsa veya köpek karede çok küçükse onu algılamayabilir. Köpeği ana özne yapan daha net bir fotoğraf deneyin.

### Uygulama çöküyor veya donuyor

App Store'dan en son sürümü çalıştırdığınızdan emin olun, ardından cihazınızı yeniden başlatıp yeniden deneyin. Sorun devam ederse lütfen aşağıdan bize ulaşın.

## Uyumluluk

Barkpedia, iOS 18.0 veya sonrasını gerektirir ve hem iPhone hem de iPad'de çalışır.

## Bize Ulaşın

Hâlâ yardıma mı ihtiyacınız var? Sizden haber almaktan memnuniyet duyarız.

**E-posta:** [contact@bartlettbutter.com](mailto:contact@bartlettbutter.com)

Genellikle 48 saat içinde yanıt veririz. Sorununuzu hızlıca çözmemize yardımcı olmak için lütfen şunları ekleyin:

- Sorunun kısa bir açıklaması
- Cihaz modeliniz ve iOS sürümünüz (Ayarlar → Genel → Hakkında)
- Varsa bir ekran görüntüsü
