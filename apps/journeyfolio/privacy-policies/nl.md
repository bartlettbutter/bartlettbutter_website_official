---
layout: default
title: Journeyfolio
app_icon: /assets/app-icons/icon_Journeyfolio.png
app_description: "Privacybeleid"
lang: nl
permalink: /journeyfolio/privacy/nl/
redirect_from:
  - /privacy-policies/journeyfolio/nl/
---

**Ingangsdatum:** 22 juli 2026

Journeyfolio ("de app") is ontwikkeld door Bartlett Butter. Dit beleid legt uit hoe de app met uw informatie omgaat. Het korte antwoord: we verzamelen er geen.

## In Één Oogopslag

| Vraag | Antwoord |
|-------|----------|
| Verzamelen jullie persoonsgegevens? | Nee |
| Zijn er accounts of aanmeldingen? | Nee |
| Is er tracking of analyse? | Nee |
| Zijn er advertenties? | Nee |
| Worden mijn gegevens verkocht of gedeeld? | Nee |
| Heeft de app toegang tot mijn GPS-locatie? | Nee |
| Is de app veilig voor kinderen? | Ja |

## Welke Gegevens We Verzamelen

**Geen.** Journeyfolio verzamelt, bewaart of verzendt geen persoonsgegevens naar onze servers. Er zijn geen gebruikersaccounts, geen analyse-SDK's, geen advertentieframeworks en geen tracking van welke aard dan ook.

## Uw Reisgegevens

Alle informatie die u aanmaakt — bestemmingen, data, activiteiten, vluchtnummers, hotelgegevens, bijgevoegde documenten en notities — wordt **alleen lokaal op uw apparaat** opgeslagen. Deze gegevens verlaten nooit uw apparaat, behalve wanneer de app communiceert met diensten van derden (hieronder beschreven) om inhoud op te halen die u expliciet hebt aangevraagd.

## Abonnementen en Aankopen

Journeyfolio biedt optionele automatisch verlengende abonnementen die volledig via de App Store van Apple worden verwerkt. We ontvangen uw betalingsgegevens niet en hebben er geen toegang toe. Alle facturering en abonnementsbeheer worden door Apple afgehandeld volgens hun [Privacybeleid](https://www.apple.com/legal/privacy/).

## Locatiegegevens

Journeyfolio heeft **geen** toegang tot de GPS van uw apparaat en volgt niet uw fysieke locatie.

Wanneer u een bestemming toevoegt of naar plaatsen zoekt, worden uw zoekopdrachten naar de servers van Apple verzonden om resultaten terug te geven. Plaatsnamen worden naar Apple verzonden voor geocodering en weergegevens. Landcodes worden naar Nager.Date verzonden voor informatie over feestdagen. Er worden geen andere locatiegegevens verzameld of verzonden.

## Apparaattoestemmingen

| Toestemming | Wanneer Gevraagd | Doel | Verlaten Gegevens het Apparaat? |
|-------------|------------------|------|----------------------------------|
| Camera | Wanneer u een document fotografeert | Reisdocumenten aan activiteiten koppelen | Nee — op het apparaat verwerkt via OCR |
| Fotobibliotheek | Wanneer u een bestaande afbeelding importeert | Opgeslagen afbeeldingen aan activiteiten koppelen | Nee |

Beide toestemmingen zijn optioneel. De app werkt volledig zonder deze. Er worden nooit afbeeldingen naar externe servers geüpload.

## Diensten van Derden

De app maakt uitsluitend verbinding met de volgende diensten om de door u aangevraagde inhoud weer te geven. Er worden geen persoonsgegevens of apparaat-ID's verzonden — alleen de minimale informatie die nodig is om aan elk verzoek te voldoen.

| Dienst | Doel | Verzonden Gegevens | Privacy-info |
|--------|------|--------------------|--------------|
| Apple Weer & Kaarten | Weersvoorspellingen, locatiezoekopdrachten, tijdzoneresolutie | Plaatsnamen, coördinaten | [Privacybeleid van Apple](https://www.apple.com/legal/privacy/) |
| OpenWeatherMap | Reserveweergegevens | Plaatsnamen | [Privacybeleid van OpenWeatherMap](https://openweather.co.uk/privacy-policy) |
| FlightAware | Verificatie van vluchtschema's en livestatus | Vluchtnummers, data | [Privacybeleid van FlightAware](https://www.flightaware.com/legal/privacy/) |
| Nager.Date | Informatie over feestdagen | Landcodes, jaar | [Nager.Date](https://date.nager.at) |

## Gegevens Delen

We verkopen, verhuren, licentiëren of delen uw gegevens niet met derden. De enige netwerkcommunicatie die de app uitvoert, zijn de hierboven vermelde serviceverzoeken.

## Bewaring en Verwijdering van Gegevens

Uw reisgegevens blijven op uw apparaat staan totdat u ervoor kiest ze te verwijderen — hetzij binnen de app, hetzij door deze te de-installeren. Bij de-installatie worden alle opgeslagen gegevens permanent verwijderd. Er is geen cloudback-up of -synchronisatie.

**U heeft altijd de controle.** Verwijder elke reis of activiteit op elk moment door binnen de app naar links te vegen.

## Uw Rechten

Omdat Journeyfolio alle gegevens lokaal opslaat en wij aan onze kant niets verzamelen, heeft u al volledige controle over uw informatie. U kunt uw gegevens op elk moment rechtstreeks binnen de app bekijken, bewerken of verwijderen — zonder verzoek aan ons.

## Privacy van Kinderen

Journeyfolio verzamelt niet bewust informatie van iemand, ook niet van kinderen jonger dan 13 jaar (of de toepasselijke minimumleeftijd in uw rechtsgebied). Omdat de app geen persoonsgegevens verzamelt, is deze veilig voor gebruikers van alle leeftijden.

## Internationale Gebruikers

Er vindt geen grensoverschrijdende overdracht van persoonsgegevens plaats omdat er geen persoonsgegevens worden verzameld. Netwerkverzoeken aan diensten van derden bevatten alleen bestemmingsgerelateerde zoekopdrachten (plaatsnamen, vluchtnummers, landcodes) en geen persoonsgegevens of apparaat-ID's, ongeacht uw locatie.

## Wijzigingen in Dit Beleid

Als we dit beleid bijwerken, wordt de herziene versie op deze pagina geplaatst met een nieuwe ingangsdatum. Materiële wijzigingen worden bovenaan de pagina vermeld. Voortgezet gebruik van de app na een update houdt aanvaarding van het herziene beleid in.

## Neem Contact Op

Vragen of zorgen over dit privacybeleid?

**E-mail:** [contact@bartlettbutter.com](mailto:contact@bartlettbutter.com)
