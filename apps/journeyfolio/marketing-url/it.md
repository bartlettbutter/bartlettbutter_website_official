---
layout: default
title: Journeyfolio
app_icon: /assets/app-icons/icon_Journeyfolio.png
app_description: "Pianifica in modo più intelligente. Viaggia più leggero. Tutti i tuoi viaggi, un unico bellissimo hub."
app_store_url: "https://apps.apple.com/us/app/journeyfolio/id6762172188"
lang: it
permalink: /journeyfolio/it/
redirect_from:
  - /marketing-url/journeyfolio/it/
---

# Ogni viaggio merita un folio.

Smetti di destreggiarti tra screenshot, e-mail sparse e fogli di calcolo lasciati a metà. Journeyfolio riunisce l'intero viaggio in un itinerario splendidamente organizzato che raccoglie voli, hotel, documenti, meteo in tempo reale e stato dei voli in tempo reale, così puoi concentrarti sul viaggio stesso.

[![Download on the App Store](/assets/badges/download-on-the-app-store.svg)](https://apps.apple.com/us/app/journeyfolio/id6762172188)

## Perché i viaggiatori scelgono Journeyfolio

- **Un hub per tutto** — Voli, hotel, trasporti, ristoranti, biglietti, assicurazioni e note organizzati giorno per giorno così nulla sfugge.
- **Documenti a portata di mano** — Scatta una foto della carta d'imbarco, della ricevuta o della conferma e sarà allegata proprio dove ti serve. L'OCR intelligente compila automaticamente i dettagli.
- **Sempre un passo avanti** — I badge di stato dei voli in tempo reale e le previsioni meteo a 5 giorni ti tengono informato senza cambiare app.
- **I tuoi dati restano tuoi** — Nessun account, nessuna analisi, nessuna pubblicità. Tutto risiede sul tuo dispositivo e funziona offline.

## Novità

Journeyfolio ora è **scaricabile gratuitamente** con abbonamenti premium opzionali per funzioni avanzate.

- **Gratis con piani flessibili.** Gli acquirenti esistenti mantengono l'accesso premium permanente, senza alcuna azione necessaria.
- **Allega documenti di viaggio** a qualsiasi attività. L'OCR intelligente legge automaticamente carte d'imbarco e conferme di prenotazione.
- **Supporto completo per iPad** con layout adattivi, Split View e multitasking con Slide Over.
- **Affidabilità, accessibilità e stabilità generale migliorate** su tutta la linea.

## Pianifica qualsiasi viaggio, a modo tuo

### Itinerari giorno per giorno

Crea un programma completo con sette categorie di attività: voli, hotel, trasporti, ristoranti, luoghi e biglietti, assicurazioni e altro. Viaggi in più città? Ogni destinazione ha la propria sezione, il proprio meteo e le festività locali.

### Tracciamento voli in tempo reale

Aggiungi un numero di volo e ottieni lo stato in tempo reale (In orario, In ritardo, Cancellato, Atterrato o In volo) visualizzato direttamente nel tuo itinerario con gli orari nel fuso orario locale di ciascun aeroporto.

### Previsioni meteo accurate

Visualizza le condizioni attuali e una previsione affidabile a 5 giorni per ogni destinazione. Basato su [Apple Meteo](https://weatherkit.apple.com/legal-attribution.html) con un servizio meteo aggiuntivo come riserva, così sai sempre cosa mettere in valigia.

### Biglietti e documenti in un unico posto

Carica carte d'imbarco, conferme di prenotazione, ricevute e documenti di viaggio direttamente nelle attività. Scatta una foto o importa un file, e tutto resta organizzato e accessibile quando ne hai più bisogno.

### Design bello e senza ingombri

Un'interfaccia minimalista che rende la pianificazione senza sforzo. Viaggi organizzati per anno, attività categorizzate in modo intuitivo e ogni dettaglio accessibile quando sei in movimento.

## Come funziona

1. **Crea un viaggio** — aggiungi un nome e una o più città di destinazione.
2. **Crea il tuo itinerario** — registra voli, hotel, ristoranti e attività giorno per giorno.
3. **Allega documenti** — scatta o importa carte d'imbarco, ricevute e conferme.
4. **Resta informato** — controlla il meteo in tempo reale, verifica i voli e visualizza le festività.
5. **Viaggia con sicurezza** — accedi a tutto offline, in qualsiasi momento e ovunque.

## Inizia gratis. Passa a premium quando sei pronto.

Pianifica fino a 10 viaggi senza costi. Sblocca viaggi illimitati, tracciamento voli in tempo reale e previsioni meteo con Premium, disponibile come abbonamento a rinnovo automatico mensile, semestrale o annuale.

## Compatibilità

iOS 18.0 o successivo · iPhone e iPad

## Privato per impostazione predefinita

Tutti i dati di viaggio restano sul tuo dispositivo. Nessun account, nessuna analisi, nessuna pubblicità, nessuna raccolta di dati. Punto.

Leggi la nostra [Informativa sulla Privacy](https://www.bartlettbutter.com/journeyfolio/privacy/) completa.

## Hai bisogno di aiuto?

Visita la nostra [Pagina di Supporto](https://www.bartlettbutter.com/journeyfolio/support/) o inviaci un'e-mail all'indirizzo [contact@bartlettbutter.com](mailto:contact@bartlettbutter.com).
